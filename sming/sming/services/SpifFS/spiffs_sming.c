...
static void spiffs_mount_internal(spiffs_config *cfg, bool reqFormat)
{
  if (cfg->phys_addr == 0)
  {
...
void spiffs_mount_noformat()
{
	  spiffs_config cfg = spiffs_get_storage_config();
	  spiffs_mount_internal(&cfg, true);
}

void spiffs_mount()
{
  spiffs_config cfg = spiffs_get_storage_config();
  spiffs_mount_internal(&cfg, false);
}
